...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 🔹 **1️ Charger les sujets depuis l'API**
    ...
    // 🔹 **2 Afficher les sujets dans la liste**
    function renderSubjects(subjects) {
        ...
        // Sélection automatique du premier sujet si disponible
      /*  if (subjects.length > 0) {
            selectSubject(subjects[0].id);
        }*/
    }

    // 🔹 **3️ Sélectionner un sujet et établir la connexion WebSocket**
    function selectSubject(subjectId) {
       ...
       /* document.querySelectorAll(".chat-item").forEach(item => item.classList.remove("active"));
        document.querySelector(`.chat-item[data-subject-id="${subjectId}"]`).classList.add("active");*/

        //  Obtenir le token et connecter le WebSocket
        getWebSocketToken(subjectId);
    }

    // 🔹 **4️ Obtenir le token WebSocket**
    ...
    // 🔹 **5️⃣ Établir la connexion WebSocket**
